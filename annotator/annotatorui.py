...
class AnnotatorUI(Frame):
	...
	def getLabelsValues(self):
		data = dict()
		for i in range(len(self.labelEntries)):
			currentEntry = self.labelEntries[i]
			id = self.labelIds[i]
	...
	def left(self):
		try:
			if self.leftCallback is not None:
				self.leftCallback()
		except Exception as e:
			logException(e, self)
	...
	def right(self):
		try:
			if self.rightCallback is not None:
				self.rightCallback()
		except Exception as e:
			logException(e, self)
